...
type UserArticleCollectModel struct { //用户收藏文章表.目前只能收藏到一个收藏夹，后续可能会改为多对多关系
	UserID       uint         `gorm:"uniqueIndex:idx_user_article_collect_userid" json:"userID"`       // 用户id
	UserModel    UserModel    `gorm:"foreignKey:UserID" json:"-"`                                      // 收藏者
	ArticleID    uint         `gorm:"uniqueIndex:idx_user_article_collect_articleid" json:"articleID"` // 文章id
	ArticleModel ArticleModel `gorm:"foreignKey:ArticleID" json:"-"`                                   // 被收藏的文章
	CollectID    uint         `gorm:"uniqueIndex:idx_collect" json:"collectID"`                        // 收藏夹的id
